...
def render_frame(env, current_dist_to_goal, vla_token="OPEN_WAREHOUSE"):
    """
    Renders the current environment state into a PIL Image with VLA Guard overlay.
    """
    fig, ax = plt.subplots(figsize=(6.5, 6.5), dpi=100)
    ax.set_xlim(0, env.width)
    ax.set_ylim(0, env.height)
    ax.set_aspect('equal')
    ax.set_title("VLA-Guarded D3QN Navigation", fontsize=13, fontweight='bold')
    ...
    # 6. Info Overlay including High-Level VLA Guard Token
    ...
    overlay_text = (f"Step: {env.step_count:3d} | Goal Dist: {current_dist_to_goal:.2f}m\n"
                    ...
                    f"VLA Guard Token: [{vla_token}]")
                    
    ax.text(0.3, env.height - 0.7, 
            overlay_text, 
            fontsize=8.5, fontfamily='monospace', fontweight='bold',
            bbox=dict(facecolor='white', alpha=0.9, edgecolor=badge_color, linewidth=2, boxstyle='round,pad=0.5'))
...
def evaluate_agent(model_path="checkpoints/best_model.pth", num_episodes=5, max_steps=250, save_dir="plots"):
    ...
    if os.path.exists(model_path):
        try:
            ...
            model.eval()
            print(f"Loaded trained VLA-Guarded model checkpoint from: {model_path}")
        except Exception as e:
            print(f"WARNING: Could not load exact checkpoint weights ({e}). Running evaluation with initialized weights.")
            model.eval()
    else:
        print(f"WARNING: Checkpoint {model_path} not found! Running evaluation with initialized weights.")
        model.eval()
    ...
    for ep in range(1, num_episodes + 1):
        ...
        if ep == 1:
            ...
            frames.append(render_frame(env, dist_to_goal, token))
        ...
        while not done:
            ...
            if ep == 1:
                ...
                frames.append(render_frame(env, info["distance_to_goal"], token))
        ...
        if info["success"]:
            ...
            if not has_saved_gif:
                best_episode_frames = first_episode_frames if ep == 1 else frames
                has_saved_gif = True
        elif info["collision"]:
            collision_count += 1
            status = "COLLISION"
    ...
    if len(gif_frames) > 0:
        ...
        print(f"Saved VLA-Guarded navigation demonstration GIF to {gif_path}")
